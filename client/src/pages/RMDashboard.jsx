...
function Overview({ users, customers, services }) {
  const cards = [
    { label: 'Users in Zone', value: users.length, color: '#16a34a' },
...
function UsersTab({ users }) {
  return (
    <div className="card" style={{ overflowX: 'auto' }}>
      ...
      <table>
        <thead>
          <tr><th>Name</th><th>Employee ID</th><th>Mobile</th><th>Role</th><th>Branch</th><th>Status</th></tr>
        </thead>
        <tbody>
          {users.length === 0 && (
            <tr><td colSpan={6} className="text-center" style={{ color: '#6b7280' }}>No users found</td></tr>
          )}
          {users.map((u) => (
            <tr key={u._id}>
              <td>{u.name}</td>
  ...
  return (
    <div className="card" style={{ overflowX: 'auto' }}>
      ...
      {loading ? <p>Loading...</p> : (
        <table>
          <thead>
            <tr><th>Name</th><th>Employee ID</th><th>Mobile</th><th>Role</th><th>Branch</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {pending.length === 0 && (
              <tr><td colSpan={6} className="text-center" style={{ color: '#6b7280' }}>No pending approvals</td></tr>
            )}
            {pending.map((u) => (
              <tr key={u._id}>
                <td>{u.name}</td>
...
function CustomersTab({ customers }) {
  return (
    <div className="card" style={{ overflowX: 'auto' }}>
      <h3>Zone Customers</h3>
      <table>
        <thead>
          <tr><th>Name</th><th>Mobile</th><th>Branch</th><th>Added By</th><th>Date</th></tr>
        </thead>
        <tbody>
          {customers.length === 0 && (
            <tr><td colSpan={5} className="text-center" style={{ color: '#6b7280' }}>No customers found</td></tr>
          )}
          {customers.map((c) => (
            <tr key={c._id}>
              <td>{c.name}</td>
...
function ServicesTab({ services }) {
  ...
  return (
    <div className="card" style={{ overflowX: 'auto' }}>
      ...
      <table>
        <thead>
          <tr><th>Customer</th><th>Mobile</th><th>Title</th><th>Status</th><th>Branch</th><th>Technician</th><th>Date</th></tr>
        </thead>
        <tbody>
          {services.length === 0 && (
            <tr><td colSpan={7} className="text-center" style={{ color: '#6b7280' }}>No services found</td></tr>
          )}
          {services.map((s) => (
            <tr key={s._id}>
  ...
  return (
    <>
      ...
      <div className="container" style={{ marginTop: 24 }}>
        <h1 className="page-title">Regional Manager Dashboard</h1>
        {user?.zoneId && (
          <div className="alert alert-info" style={{ marginBottom: 16 }}>
            Zone ID: <strong>{typeof user.zoneId === 'object' ? user.zoneId.name : user.zoneId}</strong>
          </div>
        )}
        <div className="tabs">
          {TABS.map((t) => (
